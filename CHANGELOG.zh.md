...
- 中央技能库顶部新增 **同步全部外部技能** 按钮，可一键同步所有带来源记录的外部技能。
- 批量同步复用单个技能的来源同步逻辑，只处理已记录 `source_path` 的技能；没有来源记录的中央技能会被跳过，不会被误改。

## 0.9.5 - 2026-06-16

新功能版本：中央技能库现在可以管理从项目技能库导入的外部技能来源，并支持从原始来源一键同步更新。

### 新功能

- **中央技能来源记录**：从项目技能库导入到中央技能库时，会记录该技能的原始目录路径，后续中央技能卡片可以识别它来自哪里。
- **从来源同步更新**：带来源记录的中央技能卡片新增同步按钮，可从原始 `SKILL.md` 所在目录重新导入内容，替换中央技能库中的旧版本。
- **外部 CLI 安装提示**：中央技能库顶部新增说明，明确 `npx skills@latest add ...` 这类外部安装只有写入当前中央目录时才会出现在中央库；否则应先在项目技能库扫描后导入。

### 改进

- 项目技能库导入逻辑改为读取用户配置的中央技能目录，不再退回默认路径。
- 中央技能列表返回来源字段，包括来源类型、来源引用、来源路径和同步时间，供 UI 判断是否显示同步入口。
- 同步更新会先解析来源目录中的 `SKILL.md` frontmatter，确认内容有效后再替换中央目录，避免把无效目录写进中央技能库。

### 后端

- 新增 `skill_sources` 表，用于保存中央技能与外部来源之间的关系。
- 新增 `sync_central_skill_from_source` Tauri 命令，负责校验来源、备份旧目录、复制新内容并更新同步时间。
- 删除或清理技能时同步清理对应来源记录，避免留下孤儿数据。

### 测试

- 新增中央技能 UI 测试，覆盖外部 CLI 提示和来源同步按钮显示。
- 新增 store 测试，覆盖 `syncSkillFromSource` 调用状态。
- 新增 Rust 测试，覆盖来源记录、导入持久化和从来源同步更新流程。
